    ...
    public static class Damage
    {
        ...
        public static double GetAutoAttackDamage(this Obj_AI_Base source, Obj_AI_Base target)
        {
            ...
            if (hero != null)
            {
                ...
                {
                    case "Aatrox":
                        if (hero.HasBuff("AatroxWPower") && hero.HasBuff("AatroxWONHPowerBuff"))
                        {
                            dmgPhysical += 10 + (35 * hero.SpellBook.GetSpell(SpellSlot.E).Level) + (hero.BonusAttackDamage * 0.75);
                        }
                        break;
                    case "Akali":
                        if (target.HasBuff("AkaliMota"))
                        {
                            dmgMagical += 20 + ( 25 * hero.SpellBook.GetSpell(SpellSlot.Q).Level) + (hero.TotalAbilityDamage * 0.5);
                        }
                        break;
                    case "Alistar":
                        if (hero.HasBuff("alistartrample"))
                        {
                            dmgMagical += 40 + 15 * hero.Level;
                        }
                        break;
                    case "Ashe":
                        if (hero.HasBuff("asheqbuff"))
                        {
                            dmgPhysical *= (0.20 + (hero.SpellBook.GetSpell(SpellSlot.Q).Level / 100)) * hero.TotalAttackDamage;
                        }
                        if (target.HasBuff("ashepassiveslow"))
                        {
                            dmgPhysical += (10 + (hero.Crit) * (1 + hero.CritDamageBonus)) * hero.TotalAttackDamage;
                        }
                        break;
                    case "Bard":
                        if (hero.GetBuffCount("bardpspiritammocount") > 0)
                        {
                            var bardpspiritBuffCount = targetHero.GetBuffCount("bardpspiritammocount");
                            dmgMagical += 30 + (15 * (bardpspiritBuffCount/5)) + 0.30 * hero.TotalAbilityDamage;
                        }
                        break;
                    case "Blitzcrank":
                        if (hero.HasBuff("PowerFist"))
                        {
                            dmgPhysical += hero.TotalAttackDamage;
                        }
                        break;
                    // todo add, when it a alies adding the last stack
                    case "Braum":
                        if (target.GetBuffCount("braummarkcounter") == 3)
                        {
                            dmgMagical += 26 + (10 * (hero.Level - 1));
                        }
                        break;
                    case "Caitlyn":
                        ...
                        break;
                    case "Camille":
                        if (hero.HasBuff("camiller"))
                        {
                            dmgMagical += 5 * hero.SpellBook.GetSpell(SpellSlot.R).Level + (2 + (2 * hero.SpellBook.GetSpell(SpellSlot.R).Level));
                        }
                        break;
                    case "Chogath":
                        if (hero.HasBuff("VorpalSpikes"))
                        {
                            dmgMagical += 5 +(15 * hero.SpellBook.GetSpell(SpellSlot.E).Level);
                        }
                        break;
                    case "Corki":
                        dmgPhysical /= 2;
                        ...
                        break;
                    case "Darius":
                        if (hero.HasBuff("DariusNoxianTacticsONH")) //W
                        {
                            dmgPhysical += 0.40 * hero.TotalAttackDamage;
                        }
                        if (hero.HasBuff("DariusNoxianMight")) //P NOXIAN MIGHT
                        {
                            dmgPhysical += new[] { 30, 30, 30, 35, 35, 35, 40, 50, 60, 70, 80, 90, 100, 120, 140, 160, 180, 200}[hero.Level - 1];
                        }
                        break;
                    case "Diana":
                        ...
                        break;
                    case "DrMundo":
                        if (hero.HasBuff("Masochism"))
                        {
                            var dmgHealthBased = (0.025 + (0.005 * hero.SpellBook.GetSpell(SpellSlot.E).Level)) * hero.MaxHealth;
                            var dmgMundoEMin = new[] {30, 50, 70, 90, 110}[hero.SpellBook.GetSpell(SpellSlot.E).Level -1];
                            var dmgMundoEMax = new[] {60 ,100, 140, 180, 220}[hero.SpellBook.GetSpell(SpellSlot.E).Level - 1];
                            if (dmgHealthBased <= dmgMundoEMin)
                            {
                                dmgPhysical += dmgMundoEMin;
                            }
                            else if (dmgHealthBased > dmgMundoEMin & dmgHealthBased < dmgMundoEMax)
                            {
                                dmgPhysical += dmgHealthBased;
                            }
                            else
                            {
                                dmgPhysical += dmgMundoEMax;
                            }
                        }
                        break;
                    case "Draven":
                    ...
                    case "Fizz":
                        ...
                        {
                            dmgMagical += 10 + (10 * hero.SpellBook.GetSpell(SpellSlot.Q).Level) + (0.4 * hero.TotalAbilityDamage);
                        }
                        break;
                    case "Gangplank": //todo add option/variable for true damage
                        if (hero.HasBuff("gangplankpassiveattack"))
                        {
                            // dmgTrueDamage += 20 + (10 * hero.Level) + hero.BonusAttackDamage;
                        }
                        break;
                    case "Garen":
                        if (hero.HasBuff("GarenQ"))
                        {
                            dmgPhysical += 5 + 25 * hero.Spellbook.GetSpell(SpellSlot.Q).Level + 0.4 * hero.TotalAttackDamage;
                        }
                        break;
                    case "Gnar":
                        if (target.GetBuffCount("gnarwproc") == 2)
                        {
                            dmgMagical += (10 * hero.Spellbook.GetSpell(SpellSlot.W).Level) + (4 + (2 * hero.Spellbook.GetSpell(SpellSlot.W).Level));
                        }
                        break;
                    case "Gragas":
                        if (hero.HasBuff("gragaswattackbuff"))
                        {
                            dmgMagical += -10 + (30 * hero.Spellbook.GetSpell(SpellSlot.W).Level) + (0.08 * target.MaxHealth) + (0.30 * hero.TotalAbilityDamage) ;
                        }
                        break;
                    case "Graves": //todo crit modifier, damage reduction against turrets and division by 3 for addition hit from the same shell
                        dmgPhysical += (new[] { 70, 71, 72, 74, 75, 76, 78, 80, 81, 83, 85, 87, 89, 91, 95, 96, 97, 100 }[hero.Level - 1] / 100) * hero.TotalAttackDamage;
                        break;
                    case "Hecarim":
                        if (hero.HasBuff("hecarimrampspeed"))
                        {
